...
| plan | `skills/plan/` | Plan a feature or initiative — vision-type write affinity, routing into the `sdd` or `requirements-cascade` track per `track-layer.spec` |
...
| review | `skills/review/` | Review changes since the default branch, or project health with no diff — experience-type write affinity; absorbs `audit`, routing into the `actualize` or `experience` track and labeling findings `spec-wrong` / `code-wrong` / `ok` |
...
| --- | --- | --- | --- |
| decision | `skills/_shared/tracks/decision.md` | document | classify → adr \| rfc → cascade |
