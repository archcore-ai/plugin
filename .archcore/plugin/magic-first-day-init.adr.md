...
**Relation wiring.** Init auto-adds `related`, `implements`, and `depends_on` edges among the seeded documents, so command grounding and the graph are useful immediately (the former `/archcore:context` pull is removed under v2 per `remove-context-command.adr`; CLI hooks and command grounding absorb it).
