...
**Deterministic version gate.** The skill's pre-flight compares the installed CLI against the pinned minimum through `bin/cli-gte`, which does a numeric field-by-field semver compare, prints `yes`, `no`, or `__NO_CLI__`, and always exits 0 — never in prose, because an LLM comparing `0.10.0` against `0.6.0` lexically gets it wrong. The gate moved from v0.6.0 to v0.6.1 when the claude-code nudge became the `CLAUDE.md` plus `AGENTS.md` managed blocks, to v0.6.4 when the Copilot writer stopped targeting `.vscode/mcp.json`, a surface Copilot CLI dropped in v1.0.37, in favor of the workspace-root `.mcp.json` it actually reads, and to v0.7.0 when the hook policy moved into the CLI per `cli-owns-layers-4-5.adr`: the plugin launchers `bin/pre-tool-use` and `bin/post-tool-use` delegate to CLI hook leaves that exist only from v0.7.0, and `hooks install` wires SessionStart, PreToolUse, and PostToolUse rather than SessionStart alone. Each bump keeps the preview and the writes in lockstep. The contract lives in `@test/unit/cli-gte.bats`.
