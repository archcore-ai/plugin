...
**Launcher contract.** `bin/pre-tool-use` and `bin/post-tool-use` are the same four steps: (1) source `bin/lib/normalize-stdin.sh` for the raw payload and host id, and `bin/lib/plugin-cache-guard.sh` for the misrouted-cwd guard — exit 0 silently from a plugin install directory; (2) fail OPEN when `bin/cli-gte 0.7.0` does not print `yes` — a pre-0.7 CLI has no pre/post-tool-use leaves, and its usage error would deny every matched call on Copilot; (3) map the shell host id `codex` to the CLI agent id `codex-cli` (all other ids match the CLI dialect names); (4) pipe the payload to `archcore hooks <cli-host> <event>` and propagate stdout, stderr, and the exit code unchanged. Exit 2 plus stderr IS the deny protocol on claude-compatible hosts; Copilot's deny is a `{"permissionDecision":"deny",…}` document with exit 0, emitted by the CLI's Copilot dialect. Transparency is pinned by `@test/unit/hook-launchers.bats`.

**The one exception to payload transparency — Cursor's post event.** The CLI folds only the qualified spellings of its tools (`mcp__archcore__*`, `mcp__plugin_archcore_archcore__*`, `archcore-*`, and the Gemini and OpenCode flattenings). Cursor's `afterMCPExecution` sends the bare name, so without translation the payload reaches the CLI's post-tool-use gate as a foreign tool and every Cursor document write leaves with no validation, cascade, or precision report — and nothing marks the session as unprotected. The 2026-09-07 audit reproduced this against CLI 0.8.3, 0.8.2, and 0.7.3 with `@test/fixtures/stdin/cursor/mcp-update.json` and an incomplete ADR: exit 0 and empty stdout; the same bytes with `mcp__archcore__update_document` produced the Precision advisory on all three. `bin/post-tool-use` therefore asks `@plugins/archcore/bin/lib/normalize-stdin.sh` (`archcore_cursor_qualify_mcp_tool`) for a copy of the payload with that one value qualified, and sends the copy only when ownership is proven. Ownership is `mcp_server_name` equal to `archcore` — the key the CLI installer writes and `@docs/cursor.mcp.example.json` documents. A bare name that merely matches ours proves nothing: a foreign server may register an `update_document` of its own, so a payload without the field, the shape `mcp-update.json` predates, passes through untouched and stays silent. `tool_input` and `result_json` are JSON strings in this event, so a `tool_name` inside them is escaped and the rewrite, anchored on the unescaped key and the exact bare value, cannot reach it; a payload with more than one unescaped `tool_name` key is a shape this translation was not written for and passes through untouched. `ARCHCORE_RAW_STDIN` stays the original capture. The rewrite is a spelling translation, not a gate: the CLI still decides whether the event triggers a scan, which is why a qualified `get_document` yields no advisory. Live Cursor sessions were not exercised; the composed fixtures `@test/fixtures/stdin/cursor/afterMCPExecution-update-archcore.json` and `afterMCPExecution-update-foreign.json` follow the hooks reference, and the claim covers those event shapes only.
...
| `@plugins/archcore/bin/post-tool-use` | Launcher — CLI validation + cascade + precision; on Cursor, the tool-name qualification of item 22 |
...
5. Each launcher MUST pass the stdin payload to the CLI with no byte changed, except the one value item 22 names.
...
8. A launcher MUST NOT shape output per host or decide from the tool name whether the CLI acts — both live in the CLI; the rewrite of item 22 translates a spelling and decides nothing.
...
22. WHEN a Cursor `afterMCPExecution` payload carries `mcp_server_name` `archcore` and a bare `tool_name` the archcore server registers, `bin/post-tool-use` MUST rewrite that value to `mcp__archcore__<tool>`.
23. IF that payload lacks `mcp_server_name`, names another server, or carries a qualified or unregistered `tool_name`, THEN `bin/post-tool-use` MUST pass it unchanged.
24. IF that payload holds more than one unescaped `tool_name` key, THEN `bin/post-tool-use` MUST pass it unchanged.
25. `bin/post-tool-use` MUST keep `ARCHCORE_RAW_STDIN` as the unmodified capture.
26. `bin/post-tool-use` MUST NOT rewrite a `tool_name` inside `tool_input`, `result_json`, or any other nested string.
27. IF the payload does not decode as one JSON object, THEN `bin/post-tool-use` MUST NOT alter any byte beyond the value item 22 names.
...
3. The launchers are byte-transparent to the CLI — same stdout, stderr, and exit status — pinned by `@test/unit/hook-launchers.bats` and, for the probe harness copy, `@test/unit/probe-wrapper.bats`. The single exception of item 22 is pinned in the same file: the one value rewritten with every other byte equal, and each condition of items 23, 24, 26, and 27 passing the payload through. Its observable result — the Precision advisory for an incomplete document through the real CLI — is pinned by `@test/integration/cursor-post-tool-use.bats`, whose positive case fails when the rewrite is removed and whose control case reproduces the bare-versus-qualified gap on the CLI alone.
