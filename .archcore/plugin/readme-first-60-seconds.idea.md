...
1. **Record the hero cast** with `asciinema rec readme-hero.cast --idle-time-limit 1.5`, starting from a fresh repository with `.archcore/` initialized, or recording the init as the first step. Use the prompt "record the decision to use PostgreSQL as our primary database — we considered DynamoDB and MongoDB", show the auto-invocation of `/archcore:decide`, the ADR creation, and the rule and guide follow-up offer, and keep it to 20 seconds or fewer and 12 visible lines in the final frame.
