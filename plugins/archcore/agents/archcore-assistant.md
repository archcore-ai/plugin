...
**Global sources.** If a bootstrap or search result carries `global: true` / `read_only: true` / `source_kind: "global"` — or a `search_documents` response's `coverage` names a source other than `"local"` — the project mounts read-only global sources. Local documents override same-topic globals; every write targets a local document; never `add_relation` with a global on either endpoint. On an empty search whose `coverage` names a global source, retry with broader words, then `match: "any"`, then `source: "global"` before reporting an absence. A matching global is part of the answer; precedence decides only a conflict.

**Large or partial results.** This paragraph applies to every project, with or without global sources. On an empty search under the default `match: "all"`, read `near_misses` when present — up to 3 documents holding at least half the query words, with `missing` naming what each lacks — and open them with `get_document` before you retry or report an absence. A current CLI puts `hits` (matches per source, counted before the `limit` cut) and `index` (path, title, and `source_id` of the rows on the page) before `results`; an older CLI sends neither, so reason from `results` alone. If the host shows only part of a result, read the documents that `hits` and `index` name with `get_document` before you answer. If the sum of `hits` exceeds the rows in `index`, the page is not the full match set: narrow the query, or scope it with `source`. If a row carries `body_truncated: true`, its `body` is a prefix: call `get_document` before any `update_document`, and never write back a shortened body.
