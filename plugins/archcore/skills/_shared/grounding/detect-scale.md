...
- **Giant single-domain library** (one `src/`, 60 modules): module-count triggers `large`; team may `--scale=medium` if large-mode noise is unhelpful.
...
User may force a mode with `--scale=small|medium|large` on the bootstrap invocation. When overridden, Step 0.5 announces both the detected mode and the forced mode, e.g.:

> Mode: medium (forced, detected=small). Override with `--scale=X` or drop the flag to auto-detect.
