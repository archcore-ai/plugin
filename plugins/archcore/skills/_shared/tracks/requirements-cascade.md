...
- `mode: sources` — discovery research; peer documents in recommended order: `requirements-cascade.mrd` → `requirements-cascade.brd` → `requirements-cascade.urd`.
- `mode: iso` — ISO 29148 decomposition with traceability: `requirements-cascade.brs` → `requirements-cascade.strs` → `requirements-cascade.syrs` → `requirements-cascade.srs`.

### Mode selection

The `plan` skill selects the mode from routing signals — never by asking the
user — in this order: explicit expert invocation, document-graph state, branch
state, request wording. Branch state contributes no mode signal in this track.
...
3. Request wording: market research, stakeholder alignment, discovery,
   competitive or opportunity analysis, or business analysis before committing
   to a product selects sources; regulated systems, audits, formal or traceable
   requirements, ISO 29148, multi-team projects, or distributed systems
   requiring auditable requirements selects iso.
...
- Next: exit. Report follow-ups: a `prd` via the `sdd` track (`skills/_shared/tracks/sdd.md`), or a `brs` via this track's iso mode — both through `/archcore:plan`; WHEN a `prd` on the topic already exists, link the source documents to it with `related` via `add_relation`.
