...
When the host is **copilot** (no wiring file list yet — see Phase C), the empty route has nothing to offer either; reply with exactly this and stop (no writes):

> Archcore is ready at `.archcore/`. No source code detected yet — no content to seed. Re-run `/archcore:init` after the first manifest or source file lands — the SessionStart empty-state nudge will keep pointing here until then.
